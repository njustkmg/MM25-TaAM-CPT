...
class CLAP(nn.Module):
    def __init__(
        ...
        if text_cfg.model_type == "transformer":
            ...
            self.text_branch = BertModel.from_pretrained(PRERAINEDS_PATH["bert-base-uncased"])
            self.text_transform = MLPLayers(units=[self.joint_embed_shape,
                                                   self.joint_embed_shape,
                                                   self.joint_embed_shape], dropout=0.1)
            self.text_projection = nn.Sequential(
            ...
            self.text_branch = RobertaModel.from_pretrained(PRERAINEDS_PATH['roberta-base'])
            self.text_transform = MLPLayers(units=[self.joint_embed_shape,
                                                   self.joint_embed_shape,
                                                   self.joint_embed_shape], dropout=0.1)
            self.text_projection = nn.Sequential(
            ...
            self.text_branch = BartModel.from_pretrained(PRERAINEDS_PATH['facebook/bart-base'])
            self.text_transform = MLPLayers(units=[self.joint_embed_shape,
                                                   self.joint_embed_shape,
                                                   self.joint_embed_shape], dropout=0.1)
            self.text_projection = nn.Sequential(
