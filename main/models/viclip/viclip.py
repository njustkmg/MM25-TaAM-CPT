...
class ViCLIP(nn.Module):
    ...
    def get_text_feature_features(self, input_text):

        device = next(self.text_encoder.parameters()).device
        text = self.text_encoder.tokenize(
            input_text, context_length=self.max_txt_l
        ).to(device)
        text_feature, text_features = self.text_encoder.forward_text(text)
        
        text_features /= text_features.norm(dim=-1, keepdim=True)
        text_feature /= text_feature.norm(dim=-1, keepdim=True)

        return [text, text_feature, text_features]
    ...
    def get_vid_feature_features(self, input_frames):
        ...
        clip_feat, clip_feats= self.vision_encoder.forward_video(input_frames)
